...
impl Input {
    ...
    fn props(&mut self, key: Key) -> Option<Command> {
        ...
        let cmd = match key.code {
            ...
            KeyCode::Char('L') => PropsCmd::Nudge(10 * steps),
            KeyCode::Char('H') => PropsCmd::Nudge(-10 * steps),
            KeyCode::Char('J') => PropsCmd::Move { down: true },
            KeyCode::Char('K') => PropsCmd::Move { down: false },
            KeyCode::Backspace => PropsCmd::Parent,
            KeyCode::Tab => PropsCmd::NextSection { back: key.mods.shift },
