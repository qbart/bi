...
impl Backend for Cpp {
    ...
    fn data(&self, data: &DataUnit, unit: &Unit, model: &Model, cx: &Context) -> String {
        ...
        out.push_str("\n#pragma once\n\n#include <string_view>\n#include <utility>\n");
        // The unit's header brings the standard headers, the support
        // file and the mapping's includes.
        ...
        for wire in data.types() {
            ...
            let named = Ty::Named(wire.to_string());
            let spelling = ty(&named, model, cx);
            let module = names::snake(&t.name);
            out.push_str(&format!("\nnamespace {module} {{\n"));
            for inst in data.of(wire) {
                out.push_str(&format!(
                    "\ninline const {spelling} {} = {};\n",
                    inst.name,
                    value(&named, &inst.value, model, cx)
                ));
            }
            let count = data.of(wire).count();
            out.push_str(&format!(
                "\n/// Every instance of the file, in its order, with its id.\ninline const std::pair<std::string_view, const {spelling} *> all[{count}] = {{\n"
            ));
            for inst in data.of(wire) {
                out.push_str(&format!(
                    "    {{{}, &{}}},\n",
            ...
            out.push_str("};\n");
            out.push_str(&format!(
                "\ninline const {spelling} *find(std::string_view id) {{\n    for (const auto &entry : all) {{\n        if (entry.first == id) {{\n            return entry.second;\n        }}\n    }}\n    return nullptr;\n}}\n"
            ));
            out.push_str(&format!("\n}}  // namespace {module}\n"));
        }
        if let Some(ns) = &ns {
...
inline const Weapon rusty_sword = Weapon{"Sword \"x\"", 10, Rarity::common, Vec2{0.5f, 0.0f}, {"a"}, std::nullopt, bi::Rgb{200, 200, 200}, bi::Ref<Weapon>{"rusty_sword"}, false};

inline const Weapon dagger = Weapon{"Sword \"x\"", 10, Rarity::common, Vec2{0.5f, 0.0f}, {"a"}, std::nullopt, bi::Rgb{200, 200, 200}, bi::Ref<Weapon>{"rusty_sword"}, false};

/// Every instance of the file, in its order, with its id.
inline const std::pair<std::string_view, const Weapon *> all[2] = {
    {"rusty_sword", &rusty_sword},
    {"dagger", &dagger},
};

inline const Weapon *find(std::string_view id) {
    for (const auto &entry : all) {
        if (entry.first == id) {
            return entry.second;
        }
...
"#;
    ...
    #[test]
    fn the_fixture_data_as_cpp() {
        let m = fixture::model(Lang::Cpp, true, "");
        let lm = Default::default();
        let u = &m.units[0];
        let text = Cpp.data(&u.data[0], u, &m, &cx(&lm, Some("gen.scriptableobjects")));
        assert_eq!(text, EXPECTED_DATA);

        // No pkg: no namespace of the package, only the type's.
        let text = Cpp.data(&u.data[0], u, &m, &cx(&lm, None));
        assert!(!text.contains("gen::scriptableobjects"));
        assert!(text.contains(
            "#include \"weapons.hpp\"\n\nnamespace weapon {\n\ninline const Weapon rusty_sword = "
        ));
        assert!(text.ends_with("    return nullptr;\n}\n\n}  // namespace weapon\n"));
        assert_eq!(text.matches("namespace").count(), 2);

        // The mapping's header rides along, before the namespace.
        let mapping_text = "[cpp]\nheader = \"// mine\"\n";
        let m = fixture::model(Lang::Cpp, true, mapping_text);
        let lm = mapping(mapping_text);
        let u = &m.units[0];
        let text = Cpp.data(&u.data[0], u, &m, &cx(&lm, Some("g")));
        assert!(text.contains("#include \"weapons.hpp\"\n\n// mine\n\nnamespace g {\n"));
    }
    ...
    fn a_set_field_overrides_the_default_in_the_data() {
        ...
        let m = fixture::model_of(Lang::Cpp, fixture::SCHEMA, &[&data], "");
        let lm = Default::default();
        let u = &m.units[0];
        let text = Cpp.data(&u.data[0], u, &m, &cx(&lm, None));
        assert!(text.contains(
            "inline const Weapon dagger = Weapon{\"Sword \\\"x\\\"\", 7, Rarity::common, Vec2{0.5f, 0.0f}, {}, \"n\", bi::Rgb{200, 200, 200}, bi::Ref<Weapon>{"
        ));
        assert!(
            text.contains("inline const Weapon rusty_sword = Weapon{\"Sword \\\"x\\\"\", 10, ")
        );
    }

    #[test]
    fn a_boxed_cycle_in_the_data() {
        let data = r##"{ "$dialect": "bi/1", "$schema": "weapons.bischema",
  "instances": [ { "$type": "Node", "$id": "root" } ] }"##;
        let m = fixture::model_of(Lang::Cpp, fixture::CYCLE, &[data], "");
        let lm = Default::default();
        let u = &m.units[0];
        let text = Cpp.data(&u.data[0], u, &m, &cx(&lm, None));
        assert!(
            text.contains("\nnamespace node {\n\ninline const Node root = Node{nullptr, {}};\n")
        );
        assert!(text.contains(
            "inline const std::pair<std::string_view, const Node *> all[1] = {\n    {\"root\", &root},\n};\n"
        ));
        assert!(text.contains("inline const Node *find(std::string_view id) {\n"));
