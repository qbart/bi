...
impl Rules {
    ...
    /// ancestor of one repository is not a rule about another. Outermost
    /// first, so that the nearer file wins.
    ///
    /// `core.excludesFile`, the global one, is not read: it lives behind `git
    /// config`, which means parsing git's config file and its includes, and it
    /// is the one of the four that nearly nobody sets for anything but editor
    /// backup files.
    pub fn inherited(root: &Path) -> Rules {
        let mut rules = Rules::default();
        // The repository `root` sits in — the first ancestor with a `.git`,
        // root itself included. `.git` may be a file rather than a directory,
        // which is what a worktree and a submodule have, so this asks whether
        // it is there rather than what it is.
        ...
        // `root`, whose own `.gitignore` the caller reads in its turn.
        // Outermost first, so that the nearer file wins.
